...
export const NAMESPACES = ['common', 'layout', 'library', 'device', 'settings', 'today', 'domain', 'people', 'calendar', 'chat', 'projects', 'notes'] as const

// Exported (not just module-local) so catalogue-parity.test.ts can assert the
// map itself stays in sync with NAMESPACES — see that test for why: NAMESPACES
// alone protects the *.json files on disk, but a namespace's registration here
// can be dropped in a merge without either failing (Phase 2 postmortem).
export const resources = {
  en: { common: enCommon, layout: enLayout, library: enLibrary, settings: enSettings, device: enDevice, today: enToday, domain: enDomain, people: enPeople, calendar: enCalendar, chat: enChat, projects: enProjects, notes: enNotes },
  ja: { common: jaCommon, layout: jaLayout, library: jaLibrary, settings: jaSettings, device: jaDevice, today: jaToday, domain: jaDomain, people: jaPeople, calendar: jaCalendar, chat: jaChat, projects: jaProjects, notes: jaNotes }
}
